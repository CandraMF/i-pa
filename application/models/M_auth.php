...
class m_auth extends CI_Model{
	...
	function cmbQuery($name='txtField',$value='',$query='',$param='',$Atas='Pilih',$vAtas=""){
		...
		$Input  = "<select $param name=\"$name\" id=\"$name\" class='select2-container form-control'>$Input</select>";
		return $Input;
	}
			...
			if($arSubMenu->count > 0){
				...
				foreach($arSubMenu->result as $rowSub){
					if($rowSub->link_sub=="setting_query_console"){
						$DropDownMenu.="<li class='submenu-item'><a class='sidebar-link' href='".base_url($rowSub->link_sub)."' target='_blank'>{$rowSub->nama_sub}</a></li>";	
					}else{
						$DropDownMenu.="<li class='submenu-item'><a class='sidebar-link' href='".base_url("login/pages/".$rowSub->link_sub)."'>{$rowSub->nama_sub}</a></li>";	
					}
					
					$i++;
				}
				$DropDownMenu="<ul class='submenu'>{$DropDownMenu}</ul>";				
				$classli="class='sidebar-item  has-sub'";
				$spanli="<span class='label label-important'>".($i-1)."</span>";
			}else{
				$classli="sidebar-item";$spanli="";
			}
			$ListMenu.="
				<li ".$classli.">
					
					<a class='sidebar-link' href='".base_url("login/pages/".$row->link)."' >
